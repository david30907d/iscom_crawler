...
for i in pyprind.prog_bar(tmp):
    try:
        if os.path.isfile('json/' + i[0]+'.json'):
            res = json.load(open('json/' + i[0]+'.json','r'))
        else:
            ...
            json.dump(res, open('json/' + i[0]+'.json','w'))
...
for index, (i, j) in enumerate(zip(tmp, distance)):
    if i[0] == j[0]:
        ...
        hybrid[i[0]] = hybrid.setdefault(i[0], 0) + index*0.00001
        hybrid[j[0]] = hybrid.setdefault(j[0], 0) + index*0.99999
